...
export default function Navbar({ cars }) {
  const readyCount = cars.filter(car => car.status === "Ready").length;
  ...
  const navbarStyles = {
    ...
    navList: "flex space-x-1 border border-gray-600 rounded-2xl p-1 bg-gray-100",
  ...
  const getLinkClassName = ({ isActive }) => {
    ...
    const inactiveClasses = "text-black hover:bg-white";
      ...
      <div className={navbarStyles.container}>
        <div className={navbarStyles.wrapper}>
          <div className={navbarStyles.brand}>
            <img src="/ie.webp" alt="Logo" className={navbarStyles.logo} />
            <div className={navbarStyles.brandText}>
              <div className={navbarStyles.brandTitle}>Car Garage</div>
              <div className={navbarStyles.brandSubtitle}>Repair & Maintenance</div>
            </div>
          </div>
          ...
          <nav className={navbarStyles.nav}>
            <ul className={navbarStyles.navList}>
              ...
              <li><NavLink to="/add" className={getLinkClassName}>Add Car</NavLink></li>
              <li><NavLink to="/cars" className={getLinkClassName}>Cars</NavLink></li>
              <li><NavLink to="/reports" className={getLinkClassName}>Reports</NavLink></li>
              <li><NavLink to="/services" className={getLinkClassName}>Services</NavLink></li>
            </ul>
          </nav>

          <div className={navbarStyles.actions}>
            <button className={navbarStyles.iconButton}><CogIcon /></button>
            <button className={`${navbarStyles.iconButton} relative`}>
              <BellIcon />
              {readyCount > 0 && (
